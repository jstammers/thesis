...
previously described in~\SectionRef{sec:imaging}. For the purposes of
...
non-linear least squares fit to~\EquationRef{eq:density_time} along
...
using~\EquationRef{eq:expansion_width}. The inset shows a typical
...
\sivalue{20}{\milli\gauss} would shift the \ac{vsr} along \(\vec{x}\)
...
\begin{figure}[!htbp]
    \centering
    \def\svgwidth{\columnwidth}
    \subfloat[][]{\scalebox{0.4}{\input{microwave_spectrum.pdf_tex}}\label{fig:microwave_spectrum}}
    \subfloat[][]{\scalebox{0.4}{\raisebox{4ex}{\input{microwave_transitions.pdf_tex}}}}
...
in~\SectionRef{subsec:vel_select}. For now, it suffices to
