...
\sivalue{7}{\milli\second} after initiating the molasses and a
subsequent one every \sivalue{5}{\milli\second}. Each image represents
a region of interest of dimensions \sivalue{16.3}{\mm} \(\times\)
\sivalue{23.4}{\mm} that covers the spatial extent of the atom cloud during the launch.}
...
to~\EquationRef{eq:density_time}. Along the vertical axis, it can be
...
inspection of the images in~\FigureRef{fig:molasses_launch} shows a
...
\begin{figure}[htpb]
  ...
  expansion.]{Integrated pixel count for a single image after
    \sivalue{7}{\ms} of ballistic expansion. The blue curve shows the profile along the horizontal axis
...
\sivalue{1.4}{\MHz\per\gauss}.
With an applied magnetic field on the order of \sivalue{1}{\gauss}, the
spin precession period is around \sivalue{0.7}{\us}, so the
...
\begin{figure}[!htbp]
    ...
    \caption[Ground state distribution during \trans{2}{2}
    pumping]{Population across the two hyperfine ground states after
    \trans{2}{2} pumping under various magnetic field strengths. The $\circ$
  ($\smalltriangledown$) markers indicate the population in $\ket{F=2}$ ($\ket{F=1})$. The red, blue and green series correspond to field strengths of \sivalue{-0.16}{\gauss}, \sivalue{1.67}{\gauss}, and \sivalue{3}{\gauss}, respectively.}
...
of the blue curve in~\FigureRef{fig:step2_mf0} to a
Lorentzian gives a shift from the hyperfine splitting frequency of \sivalue{5.6}{\kilo\hertz}.
The second order Zeeman shift is
\sivalue{575.15}{\hertz\per\gauss\squared}, so the corresponding field
strength is \sivalue{3.12}{\gauss}. 
\begin{figure}[!htbp]
    \centering
...
\begin{figure}[!htbp]
  ...
  populations are shown in blue, orange and green, respectively. After \sivalue{30}{\micro\second}, approximately 5\% of the population remains in the \(m_F = \pm 1\) sub-levels.}
...
\sivalue{1016}{\micro\second}. It is clear that \EquationRef{eq:damped_rabi}
