...
samples each of duration \sivalue{2}{\s}. Below \sivalue{1}{\kHz}, the
...
\sivalue{3.4}{\kHz} (and multiples thereof) are also present.
\FigureRef{fig:laser_int_psd} shows the resultant power spectral density of the interferometer
phase, obtained by multiplying the laser phase noise power spectral
...
\Phi$ is calculated using~\EquationRef{eq:phase_contrib}.
...
\sivalue{15}{\milli\radian} and a minimum Allan deviation of close to
\sivalue{10}{\milli\radian}. This fluctuation is an order of magnitude
...
\EquationRef{eq:acc_tf_low} gives a low-frequency limit of
...
The pneumatic suspension mechanically filters the acceleration noise within the 6-\sivalue{200}{\Hz}
...
\FigureRef{fig:vibration_psd_int} shows the interferometer phase power
...
\end{equation} 
\FigureRef{fig:vibration_shot_fluct} shows the calculated Allan
deviation of the interferometer phase for
increasing averaging time using the acceleration noise power spectral
densities shown in~\FigureRef{fig:vibration_psd_int}. 
At short averaging times, we can expect an Allan deviation with the
table floating of around \sivalue{250}{\milli\radian}. Without the
pneumatic support, the Allan deviation increases to
\sivalue{650}{\milli\radian}. For averaging times at least up to
\sivalue{50}{\second}, the acceleration noise remains uncorrelated and
...
\SectionRef{subsec:vibration_sensitivity} discusses a method for
...
the Raman beams can induce phase shifts through the ac Stark shift. It
is expected that these have a smaller effect on the acceleration
sensitivity than the vibration noise, and have yet to be
...
\sivalue{6000.11}{\Ohm}. The value of $\Phi_\text{off} =
$\sivalue{0.24}{\radian} is consistent with an acceleration offset of
\sivalue{24}{\micro\meter\per\second\tothe{2}}. The MEMS accelerometer
has a bias of \sivalue{2100}{\micro\g}, so it is conceivable that this
is the cause of the offset (up to the fringe period $\Delta a =$
\sivalue{64}{\micro \g}) between the phase measured by the
...
noise. \FigureRef{fig:low_vib_hist_atom_res} shows a histogram of the
...
\sivalue{1.6e-4}{\um\s\tothe{-2}}. The interferometer is less
...
\par\noindent
\FigureRef{fig:low_vib_adev} shows the Allan deviation relative to the
...
\begin{figure}[htpb!]
  ...
  environment.]{Allan deviation of the estimated acceleration using
    ...
    in~\SectionRef{subsec:vibration_sensitivity}. The orange and blue
