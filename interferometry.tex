...
\Muquans laser (see~\FigureRef{fig:muquans_cooling}). This value
...
0.83(11)\)
and . \FigureRef{fig:detection_time} shows the voltage recorded by the
...
\SectionRef{subsec:optical_pumping}, we know that the magnetic field is not
...
\begin{figure}[htpb!]
  ...
  detection times.]{Photodiode voltage during atom detection. The
    ...
    was measured for \sivalue{200}{\us}. The saturation parameters
    are \(s =
...
are \sivalue{0.95}{\mega\hertz} and \sivalue{1.895}{\mega\hertz}, and
...
in~\FigureRef{fig:raman_spectrum_inset}. The shoulder visible on the
...
in~\EquationRef{eq:diff_shift} are strictly negative. Here, we
...
intensities for each Raman beam. \FigureRef{fig:light_shift_plots}
...
obtained. \par\noindent
\FigureRef{fig:light_shift_plots} shows the differential ac
...
\sivalue{-1.13}{\GHz} and its corresponding ratio of 0.583.}
...
\sivalue{880}{\hertz}. If we assume that their shifts are entirely due
...
\sivalue{23}{\milli\radian}. This is almost certainly an overestimate,
...
\sivalue{1.56}{\gauss}, which is consistent with the value measured
previously in~\SectionRef{subsec:raman_spec}.
...
\begin{figure}[htpb]
  ...
  \caption[Rabi oscillations between the $\ket{1,0}$ and $\ket{2,0}$
    ...
    velocity-selective pulse of duration \sivalue{40}{\us} after
    launching along the Raman axis and vertically with a velocity of
    \sivalue{27}{\cm\per\s} so the atoms
  ...
  \sivalue{23}{\ms} (orange) and \sivalue{33}{\ms} (green). The atoms
  come to rest after \sivalue{27.5}{\ms} and close to the apogee, the
...
from~\FigureRef{fig:noise_source_psd}, decreases as the integration
...
\begin{figure}[htpb!]
  ...
  \caption[Detector noise vs. integration time.]{Detector noise
    ...
    from~\FigureRef{fig:noise_source_psd} up to a frequency
...
\end{equation}
\par\noindent
\FigureRef{fig:transfer_function} plots $|H(\omega)|$
using~\EquationRef{eq:sens_fourier_full} for
...
\begin{figure}[htpb!]
  ...
  \caption[Transfer function for Raman phase noise.]{Transfer function
    ...
    using~\EquationRef{eq:sens_fourier_full}. Here, the separation between pulses is \(T =
...
\FigureRef{fig:msquared_laser}). The phase is extracted from the
Fourier transform of the beat-note time series. \FigureRef{fig:laser_phase_drift}
...
\sivalue{50}{\milli\radian} are present. In spite of the slow drift in the Raman
...
\sivalue{15}{\milli\radian}. This fluctuation is an order of magnitude
...
\sivalue{0.21}{\radian}\,$N^{-1/2}$ and \sivalue{0.4}{\radian}\,$N^{-1/2}$,
respectively.
...
After 50 measurements (\sivalue{40}{\s}), the interferometer reaches a sensitivity to
acceleration of \sivalue{3}{\um\s\tothe{-2}}. This is far greater than
the current state-of-the-art (\sivalue{3e-12}{\m\s\tothe{-2}} after
\sivalue{5}{\s} reported in \cite{Barrett2016}) but given that the
first generation gravimeter reached a sensitivity of
\sivalue{3e-5}{\m\s\tothe{-2}} after \sivalue{1000}{\s}
