...
\ac{mot}~\cite{Dieckmann1998} system both increases the loading rate
and decreases the pressure inside the main chamber.  \par\noindent A diagram of the light and magnetic fields
...
\begin{figure}[!htbp]
	...
	\subfloat[][]{\scalebox{0.3}{\includegraphics{2D_field_xy.pdf}\label{fig:2d_field_xy}}}
	\subfloat[][]{\scalebox{0.3}{\includegraphics{2D_field_xz.pdf}}\label{fig:2d_field_xz}}\\
  \subfloat[][]{\scalebox{0.3}{\includegraphics{2D_grad.pdf}}\label{fig:2d_grad}}
...
the \ac{ecdl}. This produces a signal that is
...
\end{figure}
\par\noindent An \ac{eom} modulates the phase of the cooling laser to produce light for the \trans{1}{2} repump transition. This modulation creates frequency sidebands separated by integer multiples of the modulation frequency
...
in~\AppendixRef{app:serial_commands}. The command is stored in memory on-board the \ac{dds} and
...
the atoms. The coils are an inductive load, so the time taken for the
