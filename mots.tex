...
vacuum chamber and the main \ac{mot} components. In this diagram,
gravity is acting towards the bottom of the page. The chamber is made of 316L stainless
...
\begin{figure}[!htbp]
	...
	\caption[\ac{mot} system component diagram]{A diagram of the main components on the vacuum chamber used for the \ac{mot}
		...
		of the chamber. Gravity acts towards the bottom of the page, along
  the central axis of the 3D \ac{mot} quadrupole coils.}
...
\begin{figure}[!htbp]
	...
	\caption[Schematic for the 2D \ac{mot}]{Schematic diagram for the 2D
  \ac{mot}. \ac{rb87} atoms are trapped and cooled along the 2 axes
orthogonal to the long axis of the source cell. The black arrows
indicate the direction of the current through each coil. The
handedness of each circularly polarised beam is indicated and are such
that each beam drives \(\sigma^-\) transitions for an atom moving in the opposite direction. A linearly polarised push beam propagates along the longitudinal axis and is partially reflected by the silicon wafer at the opposite end. This provides a small amount of axial cooling and the imbalance of radiation pressure pushes atoms out of the cell. The pinhole at the other end prevents atoms with a high transverse velocity from leaving the cell.}
...
\sivalue{9.5}{\milli\metre} using two aspheric lenses. A \ac{hwp}
divides the 
linearly polarised light
into two beams of equal power, one for each cooling
...
mirror retro-reflects the beam. A \ac{qwp} is bonded onto the surface
of the mirror and is aligned so that the
...
\sivalue{25}{\mm} kinematic mount to align the push beam to the \sivalue{0.7}{\milli\metre} pinhole at the other end of the cell. The push beam is linearly polarised by a \ac{pbs} to reduce the effect of
...
cell and retro-reflected by a \ac{qwp} bonded on top of a mirror. The push
...
\begin{figure}[htbp!]
	...
	\subfloat[][]{\scalebox{0.35}{\includegraphics{2D_field_xz.pdf}}\label{fig:2d_field_xz}}
  \subfloat[][]{\scalebox{0.35}{\includegraphics{2D_grad.pdf}}\label{fig:2d_grad}}\\
  \subfloat[][]{\scalebox{0.35}{\includegraphics{2D_field_xy.pdf}\label{fig:2d_field_xy}}}
	\caption[Calculated field and field gradients for the 2D \ac{mot} quadrupole
		...
		\textbf{(c)}, respectively. \textbf{(b)} shows the field gradient
		components \(\partial_x B_x\) along the $x$ axis. }
...
\begin{figure}[!htbp]
	\centering
	\def\svgwidth{\columnwidth}
  \subfloat[][]{\includegraphics[width=0.45\textwidth]{mot_coil_temperature.pdf}\label{fig:mot_coil_temp}}
	\subfloat[][]{\includegraphics[width=0.45\textwidth]{mot_coil_temperature_chamber.pdf}\label{fig:mot_coil_temp_tg4}}
...
\begin{figure}[!htbp]
	\centering
	\def\svgwidth{\columnwidth}
  \subfloat[][]{\includegraphics[width=0.45\textwidth]{mot_field_3D.pdf}\label{fig:coil_a}}
  \subfloat[][]{\includegraphics[width=0.45\textwidth]{mot_field_together_3D.pdf}\label{fig:coil_b}}
	\caption[Measured magnetic field for the 3D \ac{mot}
  coils]{Measured magnetic field for the 3D \ac{mot}
		coils. \textbf{(a)} red and blue circles show the axial magnetic
    field component of both coils as
		measured using a Hall probe, where the origin corresponds to the
    outermost coil loop. \textbf{(b)} red circles show the
    field close to the centre when the coils are operating together.
    In this case, the origin corresponds to the mid-point between the
    two coils, which is the centre of the trap. The dashed lines indicate the axial field as
		calculated from the Biot-Savart law. Close to the centre of the
  coils, the magnetic field is approximately linear, indicating a
...
as a function of exposure time is plotted
in~\FigureRef{fig:camera_counts}. Each data point is obtained by
subtracting a background measurement for the same exposure time. Below
\sivalue{50}{\us}, the signal-to-noise ratio is
too low to detect the incident light as indicated by the fact that
zero counts are observed for a non-zero duration. This
...
\begin{figure}[!htbp]
	...
	\caption[Integrated pixel counts as a function of CCD exposure
		...
		incident optical power of \sivalue{0.17}{\micro\watt} after
    subtracting a background measurement. The dashed line
		indicates a linear regression which gives a scaling factor of \(\alpha\)
		=
		\pow{2.2}{5}\,counts\,\sivalue{}{\per\micro\second\per\micro\watt}.
  Zero counts are detected for a non-zero duration because the
incident light level is equal to the background light level for
exposure times below \sivalue{50}{\us}.}
...
(see text). Between
$\pm$\sivalue{3}{\MHz}, the error signal is proportional to the
frequency difference and is used by a servo-loop to keep the
difference close to zero.}\label{fig:slave_offset}
\end{figure}
\par\noindent 
...
\subsection{Optical Fibre Network}\label{subsec:optical_fibre} 
A network of fibre-based beam-splitters and \acp{aom} distributes the light from the \Muquans
...
controlled independently. The percentages shown are the fractions of
the input power
at each output, when driving each \ac{aom} with the optimum RF power.} 
...
bias coils is
controlled using a voltage-controlled current source.
\FigureRef{fig:mot_coil_circuit} shows a diagram of the circuit used
to control the current through the bias coils and 3D \ac{mot}
quadrupole coils. The control
...
coils.
\begin{figure}
  \scalebox{0.4}{\input{mot_coil_circuit.pdf_tex}}
  \caption[Circuit diagram for 3D \ac{mot} quadrupole coils and bias
  field coils.]{Circuit diagram for 3D \ac{mot} quadrupole coils and bias
  field coils. The current through the coils (shown as an inductor) is
controlled using an input voltage $V_{in}$ and a sense resistor $R_c$.}
  \label{fig:mot_coil_circuit}
\end{figure}
\par \noindent
...
same way. \FigureRef{fig:igbt_circuit} shows a diagram of the circuit
used. An IGBT cuts the flow of current when the gate voltage drops
...
\begin{figure}
  \centering
  \includegraphics[height=0.15\textheight]{igbt_circuit.pdf}
  \caption[IGBT circuit for the 2D \ac{mot} quadrupole coils]{IGBT
  circuit for the 2D \ac{mot} quadrupole coils. The circuit is broken
to switch the coils off, which generates a back-EMF. This flows
through the diode and resistor in parallel with the coils, preventing
damage to the IGBT. }
  \label{fig:igbt_circuit}
\end{figure}
...
great enough, the loss due to untrapped rubidium collisions becomes
non-negligible and the number of atoms in the
\ac{mot} becomes independent of
\(\phi_\text{rb}\). At very high pressures, this collision rate is
great enough to cause a roll-off in the number of trapped atoms. \par\noindent The flux of atoms from the 2D \ac{mot} depends on the 2D
