...
optic axis. Recalling~\SectionRef{subsec:pulse_prop}, the state of the
atom (see~\EquationRef{eq:general_state}) depends on the Rabi frequency
...
the Rabi frequency for each atom during each pulse. The contrast for
...
using~\EquationRef{eq:cloud_contrast}. The blue curve shows the contrast when the atom cloud is initially at the
...
in~\FigureRef{fig:retro_divergence}. Each Raman pulse will impress a different
...
Based on the experimental apparatus, the first set of rays propagates \sivalue{43}{\milli\metre} and the second propagates \sivalue{129}{\milli\metre}. 
...
in~\FigureRef{fig:retro_divergence}.  
\FigureRef{fig:collimation_error} shows the corresponding interferometer phase for an
atom falling from rest under gravity with a pulse separation of $T=$
\sivalue{25}{\ms}. A divergence angle of
\sivalue{350}{\micro\radian} results in an interferometer phase
contribution of \sivalue{10}{\milli\radian}. It should also be noted
...
\begin{figure}[!htbp]
  ...
  \subfloat[][]{\scalebox{0.3}{\input{retro_divergence.pdf_tex}\label{fig:retro_divergence}}}
	\subfloat[][]{\includegraphics[width=0.4\textwidth]{wavefront_longitudinal.pdf}\label{fig:collimation_wavefront}}\\
  \subfloat[][]{\includegraphics[width=0.4\textwidth]{collimation_phase_error.pdf}\label{fig:collimation_error}}
  ...
  beam divergence angles of
    ...
    \sivalue{1.2}{\milli\radian} (red). The black points indicate the
    phase shift of each interferometer pulse for an atom falling under
    gravity and a pulse separation time of \sivalue{25}{\ms}.
    ...
    \sivalue{350}{\micro\radian} results in a phase error of less
    than \sivalue{10}{\milli\radian} which is the value that
    corresponds to the uncertainty due to sources of phase noise. 
		}\label{fig:collimation_error_plots}
\end{figure}
\par\noindent
Another point to consider is the possible tilt of the Raman beams. \FigureRef{fig:perfect_retro}
...
less than \sivalue{0.1}{\micro\radian}. Clearly this level of
precision will require the transverse position of the input fibre 
(see ~\SectionRef{subsec:setup_ramancollimator}) to be exceedingly
stable.
\begin{figure}[!htbp]
	\centering
  \fontsize{12pt}{12pt}
...
in~\FigureRef{fig:imperfect_retro}. In this case the phase difference
...
acceleration of \sivalue{0.3}{\g}, a tilt
of $\theta = $ \sivalue{0.26}{\milli\radian} gives an interferometer
...
$\sigma_\Phi$ will not exceed \sivalue{0.1}{\radian} because of the
...
200--\sivalue{300}{\micro\metre}. The fibre plate is free to rotate so
...
in~\SectionRef{subsec:int_pulses}.
...
\begin{figure} 
  ...
  \sivalue{0.4\pm0.03}{\mm\per\m}, corresponding to a divergence
angle of \sivalue{0.4\pm0.03}{\milli\radian}.} 
