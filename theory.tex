...
each other if their wavefunctions overlap. The interference pattern depends on the difference in phase between the two paths.
\par\noindent
...
\end{equation}
so that its position and velocity are given by
\begin{subequations}
\begin{align}
...
indicate positions in the absence of acceleration. The last factor in
...
\begin{subequations}
  \begin{align}
    ...
    \phi_\textnormal{eff} &= \phi_1 -\phi_2 \label{eq:raman_phase_eff}\\
...
\end{equation}
Here, the second term in the square brackets is the Doppler shift for
an atom with velocity $\textbf{v}$ and
...
\trans{2}{i} transition has been neglected compared with $\Delta$. The
phase of the Rabi frequency is determined by $\phi_\textnormal{eff}$,
which was previously defined in \EquationRef{eq:raman_phase_eff}.
...
detuning from
\EquationRef{eq:raman_detuning}. If we consider an atom starting in
the state $\ket{1}$, then it is straightforward to show that after
applying $U$, the probability of stimulating a transition to $\ket{2}$
is given by
\begin{equation}
  \text{P}_{\ket{2}} = \frac{\Omega_\textnormal{eff}^2}{\Omega'^2}
  \sin^2\left(\frac{\Omega' t}{2}\right)
  \label{eq:trans_prob}
\end{equation}
\subsection{Application to a Sequence of Raman
Pulses}\label{subsec:pulse_prop}
...
e^{i \omega t}\right)$. If one beam
is left-handed and the other is right-handed, the near-resonant product of dipole operators
results in components of the form $d^{(1)}_+ d^{(2)}_+$. This
changes the magnetic quantum number by $\pm2$, which is suppressed for
an $\ket{F} \rightarrow \ket{F+1}$ transition. This can be driven at
resonance, but the transition amplitude rapidly decays as the detuning
increases.
